...
def amazon_price_checker(amazon_link, ebay_price):
    ...
    price = soup.find('a', {"href": "gp/offer"})
    print(price)
...
def other_retailer_checker(item_link, ebay_price):
    ...
    for alternate_retailer_link in other_retailer_links:
        if (alternate_retailer_link.find('/dp/') != -1):
            amazon_price_checker(alternate_retailer_link, ebay_price)
        if (alternate_retailer_link.find('walmart') != -1):
...
def eBay_item_checker(hot_items):
    ...
    for item_link in hot_items:
        ...
        item_price_ebay = (soup.find(attrs = {'class': 'notranslate'}).get_text()).lstrip('US $')
        ...
        if((int(feedback_score) > 100) and (int(feedback_score) < 10000)):
            other_retailer_checker(item_link, item_price_ebay)
